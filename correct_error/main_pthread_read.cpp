...
uint8_t* make_kmerFreq_1bit_table_from_8BitGz_pthread(string &kmer_freq_file, int Ksize, uint64_t &total, uint64_t &num_total_kmers, uint64_t &num_effect_kmers, int low_freq_cutoff, double &low_freq_ratio);
...
uint8_t *buffer_uncompress;
uint64_t *num_total_kmers_pthread;
uint64_t *num_effect_kmers_pthread;
uint64_t *low_freq_num_pthread;
uLongf *uncompLen;
ifstream Compr_KmerFreq_File;
pthread_mutex_t Block_mutex = PTHREAD_RECURSIVE_MUTEX_INITIALIZER_NP;
pthread_mutex_t Kmer_mutex = PTHREAD_RECURSIVE_MUTEX_INITIALIZER_NP;
...
void usage() 
{	cout << "\ncorrect_error_parallel (v2.2) <kmer_freq.cz> <reads_file_list>\n" 
...
{	
	...
	int c;
	while((c=getopt(argc, argv, "k:l:m:c:x:n:r:t:p:f:j:h")) !=-1) {
		switch(c) {
	...
	make_kmerFreq_1bit_table_from_8BitGz_pthread(kmer_freq_file, KmerSize, Kmer_theory_total, Kmer_total_num, Kmer_effect_num, Low_freq_cutoff, low_freq_kmer_ratio);
...
void *thread_uncompressKmerFreq(void* threadId_p)
{
	...
	{	
		...
		{	
			uint64_t this_block_id = bufBlockId ++;
			Compr_KmerFreq_File.read((char *)(buffer_compress+threadId*SrcBlockSize), BlockSizeVec[this_block_id]);
			...
			//对读入缓存的内容进行解压缩
			uncompLen[threadId] = SrcBlockSize;
			uncompress((Bytef *)(buffer_uncompress+threadId*SrcBlockSize), &uncompLen[threadId], (const Bytef*)(buffer_compress+threadId*SrcBlockSize), BlockSizeVec[this_block_id]);
			
			//找到高频的kmer位置
			uint64_t BlockStartPos = this_block_id * SrcBlockSize;
			uint8_t *buf_uncompr = buffer_uncompress + threadId * SrcBlockSize;

			vector<uint64_t> highfreqkmers;
			for (uint64_t j = 0; j < uncompLen[threadId]; j ++)
			{	uint64_t idx = BlockStartPos + j;
				if (buf_uncompr[j] > 0)
				{	num_total_kmers_pthread[threadId] += buf_uncompr[j]; 
					num_effect_kmers_pthread[threadId] ++;
					if(buf_uncompr[j] > Low_freq_cutoff)
					{	
						//只分成两个档,低频和高频, 在此处将高频转换成正负链都有的形式
						//能自动处理KmerSize为偶数的情况，正负链形式存在一个地方
						highfreqkmers.push_back(idx);
						uint64_t rc_idx = get_rev_com_kbit(idx, KmerSize);
						highfreqkmers.push_back(rc_idx);
						
					}else{
						low_freq_num_pthread[threadId] ++;
					}
				}
			}
			...
			//实现kmerfreq由byte到bit的转变，这一步多个线程可能冲突，故需要加锁
			pthread_mutex_lock(&Kmer_mutex);	
			for (uint64_t i=0; i<highfreqkmers.size(); i++)
			{	KmerFreq[highfreqkmers[i]/8] |= bitAll[highfreqkmers[i]%8];
			}
			pthread_mutex_unlock(&Kmer_mutex);
...
{	
	...
	buffer_compress = new uint8_t[SrcBlockSize*threadNum];
	buffer_uncompress = new uint8_t[SrcBlockSize*threadNum];
	num_total_kmers_pthread = new uint64_t[threadNum];
	num_effect_kmers_pthread = new uint64_t[threadNum];
	low_freq_num_pthread = new uint64_t[threadNum];
	...
	//create children threads
	for (int i=0; i<threadNum; i++)
	{	num_total_kmers_pthread[i] = 0;
		num_effect_kmers_pthread[i] = 0;
		low_freq_num_pthread[i] = 0;
		pthreadId[i] = i;
		pthread_create(pthread+i, NULL, thread_uncompressKmerFreq, (void*)(pthreadId+i));
	}
	cerr << "To uncompress the kmer-freq-table, " << threadNum << " threads were created!\n" << endl;
	...
	//join to calculate num_total_kmers, num_effect_kmers, low_freq_num
	for (int i=0; i<threadNum; i++)
	{	num_total_kmers += num_total_kmers_pthread[i];
		num_effect_kmers += num_effect_kmers_pthread[i];
		low_freq_num += low_freq_num_pthread[i];
	}
			
	delete[] buffer_compress;
	delete[] buffer_uncompress;
	delete[] num_total_kmers_pthread;
	delete[] num_effect_kmers_pthread;
	delete[] low_freq_num_pthread;
	delete[] uncompLen;
...
void *thread_mergeTwoFile(void *data)
{
	...
	while (1)
	{
		...
		{ 
			...
			string read1_file = (*file_vec_p)[File_num++] + ".cor";
			string read2_file = (*file_vec_p)[File_num++] + ".cor";
...
{	
	...
	string reads_file_cor = reads_file + ".cor";
	...
	{	
		...
		{	
			corFile << HeadStr[i]  << " score: " << Score[i] << "  left_trim: " << LeftTrim[i]  << "\n" << RawReads[i] << endl;
		}
	...
	string reads_file_cor_stat = reads_file + ".cor.stat";
